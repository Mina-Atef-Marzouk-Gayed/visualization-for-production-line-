...
class MainWindow(QMainWindow):
    ...
    def init_ui(self):
        """Initialize the user interface - now with sidebar + stacked widget"""
        ...
        # ===== HEADER BAR (unchanged except removed station control buttons) =====
        header_frame = QFrame()
        ...
        # Station process status (visualizer processes)
        ...
        hsplit_layout = QHBoxLayout()
        hsplit_layout.setSpacing(15)
        ...
        # Title
        sidebar_title = QLabel("📋 STATIONS")
        sidebar_title.setFont(QFont("Segoe UI", 12, QFont.Bold))
        sidebar_title.setStyleSheet("color: #ffffff; padding: 5px;")
        sidebar_layout.addWidget(sidebar_title)
        
        # Station list
        self.station_list = QListWidget()
        self.station_list.setFont(QFont("Segoe UI", 11))
        self.station_list.addItem("ALL")
        for sid in STATIONS.keys():
            self.station_list.addItem(sid)
        self.station_list.setCurrentRow(0)
        self.station_list.currentRowChanged.connect(self.on_station_selected)
        ...
        """)
        sidebar_layout.addWidget(self.station_list)
        
        sidebar_layout.addSpacing(20)
        
        # --- Station control buttons (moved from header) ---
        control_label = QLabel("🛠️ CONTROL")
        control_label.setFont(QFont("Segoe UI", 11, QFont.Bold))
        control_label.setStyleSheet("color: #dddddd; padding: 5px;")
        sidebar_layout.addWidget(control_label)
        
        self.start_stations_btn = QPushButton("▶ Start All")
        self.start_stations_btn.setFont(QFont("Segoe UI", 10, QFont.Bold))
        self.start_stations_btn.clicked.connect(self.start_all_stations)
        sidebar_layout.addWidget(self.start_stations_btn)
        
        self.stop_stations_btn = QPushButton("⏹ Stop All")
        self.stop_stations_btn.setFont(QFont("Segoe UI", 10, QFont.Bold))
        self.stop_stations_btn.clicked.connect(self.stop_all_stations)
        sidebar_layout.addWidget(self.stop_stations_btn)
        
        self.restart_stations_btn = QPushButton("↻ Restart All")
        self.restart_stations_btn.setFont(QFont("Segoe UI", 10, QFont.Bold))
        self.restart_stations_btn.clicked.connect(self.restart_all_stations)
        sidebar_layout.addWidget(self.restart_stations_btn)
        
        sidebar_layout.addStretch()
        
        hsplit_layout.addWidget(sidebar)
        
        # --- RIGHT STACKED WIDGET ---
        self.stack = QStackedWidget()
        self.stack.setObjectName("rightStack")
        
        # Page 0: Pipeline overview (existing pipeline widget)
        self.page_overview = QWidget()
        overview_layout = QVBoxLayout(self.page_overview)
        overview_layout.setContentsMargins(0, 0, 0, 0)
        self.pipeline_widget = PipelineWidget()
        overview_layout.addWidget(self.pipeline_widget)
        self.stack.addWidget(self.page_overview)  # index 0
        
        # Pages 1-6: Station detail pages
        self.station_pages = {}  # station_id -> widget
        self.station_page_labels = {}  # station_id -> dict of QLabel references
        for i, station_id in enumerate(STATIONS.keys(), start=1):
            page = self._create_station_page(station_id)
            self.stack.addWidget(page)
            self.station_pages[station_id] = page
        
        hsplit_layout.addWidget(self.stack, 1)  # give stack all remaining space
        main_layout.addLayout(hsplit_layout, 1)  # give hsplit stretch
        
        # ===== CONTROL BAR (unchanged, but now sits below sidebar+stack) =====
    ...
    # ----------------------------------------------------------------------
    # Helper: create a detail page for a single station
    # ----------------------------------------------------------------------
    def _create_station_page(self, station_id: str) -> QWidget:
        """Build a QWidget displaying live status for a station."""
        page = QWidget()
        ...
        # Header
        title = QLabel(f"{station_id} – Station View")
        ...
        # Style the status LEDs
        for lbl in (ready_lbl, busy_lbl, done_lbl, fault_lbl):
            lbl.setFont(QFont("Segoe UI", 10))
            lbl.setStyleSheet("padding: 2px 6px; border-radius: 8px; color: white;")
        
        status_layout.addRow("Ready:", ready_lbl)
        status_layout.addRow("Busy:", busy_lbl)
        status_layout.addRow("Done:", done_lbl)
        status_layout.addRow("Fault:", fault_lbl)
        status_layout.addRow("Cycle Time:", cycle_lbl)
        status_layout.addRow("Batch ID:", batch_lbl)
        status_layout.addRow("Last Activity:", time_lbl)
        
        layout.addWidget(status_frame)
        
        # Store label references for fast update
        self.station_page_labels[station_id] = {
            'ready': ready_lbl,
            'busy': busy_lbl,
            'done': done_lbl,
            'fault': fault_lbl,
        ...
        script_path = os.path.join(self.process_manager.working_dir, f"{station_id.lower()}_visualizer.py")
        open_btn = QPushButton(f"🔍 Open {station_id} Visualizer")
        open_btn.setFont(QFont("Segoe UI", 12, QFont.Bold))
        open_btn.setMinimumHeight(50)
        
        if os.path.exists(script_path):
            open_btn.clicked.connect(lambda checked, sid=station_id: self.process_manager.start_station(sid))
        else:
            open_btn.setText(f"❌ {station_id} Visualizer (script not found)")
            open_btn.setEnabled(False)
            open_btn.setStyleSheet("background-color: #5a2a2a;")
        layout.addWidget(open_btn)
        
        layout.addSpacing(20)
        
        # --- Notes / What's needed (editable later) ---
        notes_label = QLabel("📝 Notes / What's needed")
        notes_label.setFont(QFont("Segoe UI", 11, QFont.Bold))
        notes_label.setStyleSheet("color: #cccccc;")
        layout.addWidget(notes_label)
        
        notes_edit = QPlainTextEdit()
        notes_edit.setPlaceholderText("Enter notes for this station (e.g., expected cycle time, common faults, ...)")
        ...
        """)
        # --- Edit this later: add custom notes for each station ---
        notes_edit.setPlainText(f"# Add custom notes for {station_id} here.\n# This text is saved per station and can be edited.")
    ...
    # ----------------------------------------------------------------------
    # Slot: station selected in sidebar
    # ----------------------------------------------------------------------
    @Slot(int)
    def on_station_selected(self, row: int):
        """Switch stacked widget page."""
        self.stack.setCurrentIndex(row)
    
    # ----------------------------------------------------------------------
    # Update station detail page with live data
    # ----------------------------------------------------------------------
    def update_station_page(self, station_id: str, state: dict):
        """Refresh the labels on the station's detail page."""
        if station_id not in self.station_page_labels:
            return
        
        labels = self.station_page_labels[station_id]
        
        # Ready
        if state.get('ready', False):
            labels['ready'].setText("● READY")
            labels['ready'].setStyleSheet("background-color: #2e7d32; color: white; padding: 2px 8px; border-radius: 8px;")
        else:
            labels['ready'].setText("● idle")
            labels['ready'].setStyleSheet("background-color: #555; color: #aaa; padding: 2px 8px; border-radius: 8px;")
        
        # Busy
        if state.get('busy', False):
            labels['busy'].setText("● BUSY")
            labels['busy'].setStyleSheet("background-color: #f57c00; color: white; padding: 2px 8px; border-radius: 8px;")
        else:
            labels['busy'].setText("● idle")
            labels['busy'].setStyleSheet("background-color: #555; color: #aaa; padding: 2px 8px; border-radius: 8px;")
        
        # Done
        if state.get('done', False):
            labels['done'].setText("● DONE")
            labels['done'].setStyleSheet("background-color: #0288d1; color: white; padding: 2px 8px; border-radius: 8px;")
        else:
            labels['done'].setText("● idle")
            labels['done'].setStyleSheet("background-color: #555; color: #aaa; padding: 2px 8px; border-radius: 8px;")
        
        # Fault
        if state.get('fault', False):
            labels['fault'].setText("● FAULT")
            labels['fault'].setStyleSheet("background-color: #c62828; color: white; padding: 2px 8px; border-radius: 8px;")
        else:
            labels['fault'].setText("● ok")
            labels['fault'].setStyleSheet("background-color: #555; color: #aaa; padding: 2px 8px; border-radius: 8px;")
        
        # Cycle time
        cycle = state.get('cycle_time_ms')
        if cycle is not None:
            labels['cycle'].setText(f"{cycle:.1f} ms")
        else:
            labels['cycle'].setText("-")
        
        # Batch ID
        batch = state.get('extra', {}).get('batch_id')
        if batch:
            labels['batch'].setText(str(batch))
        else:
            labels['batch'].setText("-")
        
        # Last activity timestamp
        # We don't have a direct timestamp in state, use the last_activity from pipeline widget?
        # Simpler: show current time or a relative "just now". We'll use last_activity from pipeline's station_states.
        last_act = self.pipeline_widget.station_states.get(station_id, {}).get('last_activity', 0)
        if last_act > 0:
            ...
            if elapsed < 1:
                ...
                time_str = f"{elapsed/60:.1f}min ago"
            labels['time'].setText(time_str)
        else:
            labels['time'].setText("-")
    
    # ===== All existing methods remain unchanged from here =====
    # (start_all_stations, stop_all_stations, restart_all_stations,
    #  update_station_status_label, discover_logs, load_live_snapshot,
    #  switch_to_live, _start_station_worker, switch_to_replay,
    #  load_replay_data, _state_changed, process_new_event,
    #  on_activity_detected, on_file_reopened, update_states_from_replay,
    #  update_animation, update_event_stats, update_idle_status,
    #  update_time_label, on_live_toggled, toggle_play, on_speed_changed,
    #  on_timeline_changed, _stop_all_workers, closeEvent)
    ...
    def _start_station_worker(self, station_id: str):
        ...
        if latest_event:
            ...
            # Update station detail page
            self.update_station_page(station_id, self.last_state_snapshots[station_id])
    ...
    def process_new_event(self, event: StationEvent):
        ...
        if prev_state is None or self._state_changed(prev_state, new_state):
            ...
            # Update station detail page
            self.update_station_page(station_id, new_state)
    ...
    def update_states_from_replay(self):
        ...
        # Apply events to pipeline and station pages
        for event in events_at_time:
            ...
            self.update_station_page(event.station_id, new_state)

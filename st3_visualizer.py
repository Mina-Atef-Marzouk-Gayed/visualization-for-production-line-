#!/usr/bin/env python3
"""
ST3 Visualizer - Standalone Qt + OpenGL visualization for ST3 Electronics Wiring logs
Enhanced with realistic wiring station visualization and improved timing model
"""
...
PRIORITY_EXTRA_KEYS = ['completed', 'scrapped', 'reworks', 'cycle_time_avg_s', 
                       'batch_id', 'recipe_id', 'strain_relief_ok', 'continuity_ok']
...
# ===== CARRY-FORWARD PARSER (ST3 SPECIFIC) =====
class ST3LogParser:
    """Carry-forward parser for ST3 Electronics Wiring logs"""
    ...
    def reset(self):
        ...
        # ST3-specific patterns
        # VSI time (same as ST2)
        self.vsi_time_pattern = re.compile(r"VSI\s+time\s*:\s*(\d+)\s*ns", re.IGNORECASE)
        
        # SimPy env.now - multiple formats for ST3
        ...
        self.bool_state_inline = {
            ...
            "done": re.compile(r"\bdone\b\s*=\s*(True|False|\d+)\b", re.IGNORECASE),
            "fault": re.compile(r"\bfault\b\s*=\s*(True|False|\d+)\b", re.IGNORECASE),
        }
    
        # ST3 cycle time patterns
        ...
        # Key-value pattern for extras (ST3 specific)
        ...
        # ST3 block patterns
        self.block_start_pattern = re.compile(r"^\+=.*ST3_ElectronicsWiring.*=\+$", re.IGNORECASE)
        self.block_end_pattern = re.compile(r"^=\+=\s*$")
    ...
    def seed_from_event(self, event: ST3Event):
    ...
    def _is_valid_extra_key(self, key: str) -> bool:
        ...
        # ST3-specific quality signals should always be accepted
        if key in ['strain_relief_ok', 'continuity_ok']:
            return True
        
        # Accept keys with underscores and alphanumeric characters
    ...
    def parse_line(self, line: str) -> Optional[ST3Event]:
        line = line.strip()
        if not line:
            return None
        ...
        # ---- Outputs section: extras including ST3-specific signals ----
        if self.in_outputs_section:
            ...
            for match in self.key_value_pattern.finditer(line):
                ...
                # ST3-specific extras: strain_relief_ok, continuity_ok
                # These are required signals for ST3
                current_line_extras[key] = v
        ...
        if self.last_emitted_ts is not None:
            if timestamp <= self.last_emitted_ts:
                timestamp = self.last_emitted_ts + 1
            # Also ensure we don't jump too far ahead
            max_increment = 1_000_000_000  # 1 second max jump
            if timestamp > self.last_emitted_ts + max_increment:
                timestamp = self.last_emitted_ts + max_increment
    ...
    def get_current_state(self) -> Optional[ST3Event]:
...
# ===== LOG DISCOVERY (ST3 SPECIFIC) =====
class LogDiscoverer:
    """Finds ST3 Electronics Wiring logs with priority for check.ST3_ElectronicsWiring.log"""
    
    @staticmethod
    def find_st3_log() -> Optional[str]:
        candidates = []
        ...
        def consider_file(full_path: str, file: str):
            ...
            # ST3 station markers
            st3_ok = ("st3" in fl) or ("station3" in fl) or ("station_3" in fl) or ("station 3" in fl)
            
            # ST3 process markers: electronics, wiring
            process_ok = ("electronics" in fl) or ("wiring" in fl) or ("electronicswiring" in fl)
            
            if not (st3_ok and process_ok):
                return
            ...
            # Score based on filename relevance for ST3
            score = 0
            # Highest priority: exact match check.ST3_ElectronicsWiring.log
            if "check.st3_electronicswiring.log" == fl:
                score += 100
            # High priority: contains ST3 and ElectronicsWiring
            if "st3_electronicswiring" in fl:
                score += 50
            # Medium priority: check.*.log files (real ST3 logs)
            if fl.startswith("check") and st3_ok:
                score += 30
            # ST3 keyword bonuses
            if "electronics" in fl:
                score += 10
            if "wiring" in fl:
                score += 10
        ...
        print(f"[LogDiscoverer] ST3 candidates total = {len(candidates)}")
        ...
        print(f"Selected ST3 log: {selected}")
        return selected

# ===== TAIL WORKER (SAME AS ST2) =====
class LogTailWorker(QThread):
    new_event = Signal(ST3Event)  # Emitted for every signal line
    ...
    def __init__(self, log_path: str, seed_event: Optional[ST3Event] = None):
        super().__init__()
    ...
    def _open_file(self, seed=False):
        ...
        try:
            ...
            if seed and self.seed_event:
                self.parser.seed_from_event(self.seed_event)  # FIXED: was seed_event, should be self.seed_event
                
        except Exception as e:
            print(f"Error opening {self.log_path}: {e}")
            self.file_handle = None
    
    def reseed_parser(self, seed_event: ST3Event):
...
# ===== OPENGL WIDGET (ST3 SPECIFIC VISUALIZATION) =====
class ST3OpenGLWidget(QOpenGLWidget):
    """OpenGL visualization widget for ST3 Electronics Wiring"""
    ...
    def __init__(self, parent=None):
        ...
        # ST3 stage animation parameters - FIXED TIMING MODEL
        ...
        # New timing model variables
        self.cycle_start_s = 0.0  # When current cycle started (SimPy time)
        self.total_cycle_s = 10.0  # Default total cycle time (from cycle_time_ms)
        self.mid_cycle_attached = False  # Flag for mid-cycle attachment
        
        # ST3-specific stage names
        self.stage_names = ["Load Electronics", "Route Wires", "Connect/Crimp", "Continuity Test"]
        
        # Colors for ST3 - enhanced for realism
        self.colors = {
            "conveyor": (0.2, 0.2, 0.25, 1.0),
            "fixture": (0.3, 0.3, 0.35, 1.0),
            "pcb": (0.0, 0.5, 0.2, 1.0),
            "wire": (0.8, 0.5, 0.0, 1.0),
            "wire_red": (0.8, 0.2, 0.1, 1.0),
            "wire_blue": (0.2, 0.4, 0.8, 1.0),
            "wire_green": (0.2, 0.7, 0.2, 1.0),
            "wire_yellow": (0.9, 0.9, 0.2, 1.0),
            "wire_white": (0.9, 0.9, 0.9, 1.0),
            "connector": (0.7, 0.7, 0.2, 1.0),
            "tester": (0.0, 0.8, 1.0, 1.0),
            ...
            "anchor_point": (0.8, 0.8, 0.0, 1.0),
            "component": (0.9, 0.6, 0.1, 1.0),
            "crimp_jaw": (0.5, 0.5, 0.5, 1.0),
            "strain_green": (0.0, 0.9, 0.0, 1.0),
            "strain_orange": (1.0, 0.5, 0.0, 1.0),
            "neutral_gray": (0.5, 0.5, 0.5, 1.0),
            "spool": (0.6, 0.4, 0.2, 1.0),
            "duct": (0.4, 0.4, 0.4, 1.0),
            "tie": (0.8, 0.8, 0.8, 1.0),
            "lcd_bg": (0.1, 0.1, 0.1, 1.0),
            "lcd_text": (0.8, 1.0, 0.8, 1.0),
            "bin": (0.3, 0.2, 0.1, 1.0),
        }
        ...
        self._glut_inited = False
        
        # Wire harness configuration
        self.wire_paths = self._generate_wire_paths()
        self.wire_colors = [
            self.colors["wire"],
            self.colors["wire_red"],
            self.colors["wire_blue"],
            self.colors["wire_green"],
            self.colors["wire_yellow"]
        ]
        
        self.setMouseTracking(True)
    
    def _generate_wire_paths(self):
        """Generate 5 wire paths from spools to PCB connectors"""
        base_paths = []
        ...
        duct_entry = (-2.5, 1.8, 0.0)
        duct_exit = (0.0, 1.5, 0.0)
        ...
        for i in range(5):
            # Each wire takes a slightly different path through the duct
            duct_mid = (-1.2, 1.6, spool_positions[i][2] * 0.3)
            path = [
                spool_positions[i],
                (spool_positions[i][0] + 0.8, spool_positions[i][1] - 0.2, spool_positions[i][2]),
                duct_mid,
                duct_exit,
                pcb_connectors[i]
            ]
    ...
    def draw_cylinder(self, base, top, radius, slices=12):
        ...
        angle = math.acos(max(-1, min(1, dot))) * 180.0 / math.pi
        ...
        if abs(angle) > 0.001 and abs(angle) < 179.999:
    ...
    def draw_wire_segment(self, start, end, radius, color):
        """Draw a wire segment as a cylinder"""
        # Set color first
        glColor4f(*color)
        
        # Draw the cylinder segment
        self.draw_cylinder(start, end, radius)
        
        # Draw small spheres at joints for better appearance
        if self._glut_inited:
            glPushMatrix()
            glTranslatef(start[0], start[1], start[2])
            glutSolidSphere(radius * 1.2, 8, 8)
            glPopMatrix()
    
    def draw_wire_harness(self, progress_0_1):
        """Draw a harness of 5 wires with growing animation"""
        if not PYOPENGL_AVAILABLE:
            return
            
        for wire_idx, path in enumerate(self.wire_paths):
            # Each wire starts slightly delayed
            wire_delay = wire_idx * 0.05
            wire_progress = max(0, min(1, (progress_0_1 - wire_delay) / (1 - wire_delay)))
            
            if wire_progress <= 0:
                continue
                
            color = self.wire_colors[wire_idx % len(self.wire_colors)]
            glColor4f(*color)
            ...
            glPushMatrix()  # Push matrix for this wire
            
            target_length = total_length * wire_progress
            current_length = 0.0
            segment_drawn = False
            ...
            for i in range(len(path) - 1):
                ...
                if current_length + segment_len >= target_length:
                    ...
                    if remaining > 0 and segment_len > 0:
                        ...
                        z = seg_start[2] + t * (seg_end[2] - seg_start[2])
                        
                        # Draw partial segment
                        self.draw_wire_segment(seg_start, (x, y, z), 0.03, color)
                        segment_drawn = True
                    break
                else:
                    # Draw full segment
                    self.draw_wire_segment(seg_start, seg_end, 0.03, color)
                    segment_drawn = True
                    current_length += segment_len
            
            glPopMatrix()  # Pop matrix for this wire
    
    def draw_spool_reel(self, x, y, z, color=None):
        """Draw a wire spool reel"""
        if not PYOPENGL_AVAILABLE:
            return
        
        if color is None:
            color = self.colors["spool"]
            
        # Draw spool body (cylinder)
        glPushMatrix()
        glTranslatef(x, y, z)
        ...
        quadric = gluNewQuadric()
        gluQuadricNormals(quadric, GLU_SMOOTH)
        gluCylinder(quadric, 0.4, 0.4, 0.3, 16, 1)
        
        # Draw end caps
        glPushMatrix()
        glTranslatef(0, 0, 0.3)
        gluDisk(quadric, 0.3, 0.4, 16, 1)
        glPopMatrix()
        
        gluDisk(quadric, 0.3, 0.4, 16, 1)
        
        # Draw wire wound around
        if self._glut_inited:
            # Only use GLUT if initialized
    ...
    def draw_cable_duct(self, start, end, width, height):
        """Draw a cable duct/channel"""
        if not PYOPENGL_AVAILABLE:
            return
            
        glColor4f(*self.colors["duct"])
        
        # Draw duct as a U-shaped channel
        dx = end[0] - start[0]
        dy = end[1] - start[1]
        dz = end[2] - start[2]
        length = math.sqrt(dx*dx + dy*dy + dz*dz)
        ...
        for i in range(segments):
            glPushMatrix()
            glTranslatef(i * length / segments, 0, 0)
            
            # Draw U-channel
            glBegin(GL_QUADS)
            # Bottom
            glVertex3f(0, 0, -width/2)
            glVertex3f(length/segments, 0, -width/2)
            glVertex3f(length/segments, 0, width/2)
            glVertex3f(0, 0, width/2)
            ...
            glVertex3f(0, 0, -width/2)
            glVertex3f(length/segments, 0, -width/2)
            glVertex3f(length/segments, height, -width/2)
    ...
    def draw_connector_housing(self, x, y, z, size=0.15):
        """Draw a connector housing on PCB edge"""
        if not PYOPENGL_AVAILABLE:
            return
            
        glColor4f(*self.colors["connector"])
        
        glPushMatrix()
        glTranslatef(x, y, z)
        
        # Draw connector body
        self.draw_box(0, 0, 0, size*1.5, size*0.8, size, self.colors["connector"])
        
        # Draw pins
        glColor4f(0.9, 0.9, 0.3, 1.0)
        for i in range(3):
            pin_x = -size*0.5 + i*size*0.5
            self.draw_box(pin_x, -size*0.3, 0, size*0.1, size*0.2, size*0.1, (0.9, 0.9, 0.3, 1.0))
        
        glPopMatrix()
    
    def draw_zip_tie(self, x, y, z, progress=1.0):
        """Draw a zip tie/cable tie"""
        if not PYOPENGL_AVAILABLE:
            return
            
        glColor4f(*self.colors["tie"])
        
        # Draw tie as a small loop
        glPushMatrix()
        glTranslatef(x, y, z)
        
        # Animate tightening
        tightness = 0.1 + 0.9 * progress
        
        # Draw tie band
        glBegin(GL_QUAD_STRIP)
        for i in range(9):
            angle = i * 40 * tightness
            rad = angle * math.pi / 180.0
            glVertex3f(0.05 * math.cos(rad), 0.02, 0.05 * math.sin(rad))
            glVertex3f(0.05 * math.cos(rad), -0.02, 0.05 * math.sin(rad))
        glEnd()
        
        glPopMatrix()
    
    def draw_tester_box(self, x, y, z, continuity_ok=None):
        """Draw tester box with LCD panel"""
        if not PYOPENGL_AVAILABLE:
            return
            
        # Draw tester box
        glColor4f(0.2, 0.2, 0.3, 1.0)
        self.draw_box(x, y, z, 1.2, 0.8, 0.6, (0.2, 0.2, 0.3, 1.0))
        
        # Draw LCD panel
        lcd_y = y + 0.3
        lcd_color = self.colors["lcd_bg"]
        
        if continuity_ok == 1:
            lcd_color = (0.0, 0.3, 0.0, 1.0)  # Dark green for PASS
        elif continuity_ok == 0:
            lcd_color = (0.3, 0.0, 0.0, 1.0)  # Dark red for FAIL
        
        glColor4f(*lcd_color)
        glBegin(GL_QUADS)
        glVertex3f(x - 0.5, lcd_y, z + 0.2)
        glVertex3f(x + 0.5, lcd_y, z + 0.2)
        glVertex3f(x + 0.5, lcd_y, z - 0.2)
        glVertex3f(x - 0.5, lcd_y, z - 0.2)
        glEnd()
        ...
        # Draw status text (simulated)
        if continuity_ok is not None:
            glColor4f(*self.colors["lcd_text"])
            glPushMatrix()
            glTranslatef(x - 0.3, lcd_y + 0.01, z)
            glScalef(0.001, 0.001, 0.001)
            
            # Simple text rendering (would need GLUT text functions)
            # For now, draw a small indicator
            glPopMatrix()
            
            # Draw indicator light
            indicator_color = self.colors["pass_green"] if continuity_ok == 1 else self.colors["fail_red"]
            glColor4f(*indicator_color)
            glPushMatrix()
            glTranslatef(x + 0.3, lcd_y + 0.01, z)
            if self._glut_inited:
    ...
    def draw_probe(self, x, y, z, motion_factor=0.0):
        """Draw a test probe with motion animation"""
        if not PYOPENGL_AVAILABLE:
            return
            
        # Add vertical motion
        motion_y = math.sin(self.animation_time * 3.0 + motion_factor) * 0.1
        
        glColor4f(0.8, 0.8, 0.8, 1.0)
        
        glPushMatrix()
        glTranslatef(x, y + motion_y, z)
        ...
        glRotatef(90, 1, 0, 0)
        quadric = gluNewQuadric()
        gluCylinder(quadric, 0.02, 0.02, 0.3, 8, 1)
        gluDeleteQuadric(quadric)
        glPopMatrix()
        
        glPopMatrix()
    
    def draw_crimp_tool(self, x, y, z, progress):
        """Draw crimp tool jaws closing"""
        if not PYOPENGL_AVAILABLE:
            return
            
        # Jaws close based on progress
        jaw_offset = 0.15 * (1.0 - progress)
        
        # Left jaw
        glColor4f(0.6, 0.6, 0.6, 1.0)
        self.draw_box(x - jaw_offset - 0.1, y, z, 0.08, 0.2, 0.05, (0.6, 0.6, 0.6, 1.0))
        ...
        self.draw_box(x, y + 0.15, z, 0.4, 0.1, 0.1, (0.4, 0.4, 0.4, 1.0))
    
    # ===== UPDATED TIMING MODEL =====
    def compute_active_stage(self, visual_time_s=None):
        """Compute which ST3 stage is active using improved timing model"""
        if visual_time_s is None:
            visual_time_s = self.animation_time
            
        # If not busy, no active stage
        if not self.current_state['busy']:
            self.active_stage_index = -1
            self.stage_progress = 0.0
            self.proc_active = False
            self.mid_cycle_attached = False
            return -1
        
        # Calculate time elapsed in current cycle
        cycle_elapsed = visual_time_s - self.cycle_start_s
        
        # If we have a total cycle time, use it to determine stage
        if self.total_cycle_s > 0:
            # Calculate normalized progress through entire cycle
            if self.total_cycle_s > 0:
                cycle_progress = cycle_elapsed / self.total_cycle_s
            else:
                cycle_progress = 0.0
            
            # Handle cycling if busy stays on longer than cycle time
            if cycle_progress > 1.0:
                # Loop in stage 4 (Continuity Test) with tester animation
                self.active_stage_index = 3
                # Create looping animation for tester
                loop_time = (cycle_elapsed % 2.0) / 2.0  # 2 second loop
                self.stage_progress = 0.7 + 0.3 * math.sin(loop_time * math.pi)
                return 3
            
            # Determine stage based on cycle progress
            stage_progress = cycle_progress * 4.0  # 4 stages
            
            self.active_stage_index = min(3, int(stage_progress))
            self.stage_progress = stage_progress - self.active_stage_index
        else:
            # Fallback to old method if no cycle time
            ...
            if visual_time_s >= self.proc_end:
                if self.current_state['busy']:
                    self.active_stage_index = 3
                    self.stage_progress = (math.sin(visual_time_s * math.pi) * 0.5 + 0.5) * 0.7
                    return 3
                else:
                    self.proc_active = False
                    self.active_stage_index = -1
    ...
    def update_state(self, event: ST3Event, mode: str = "LIVE", event_time_s: float = None):
        """Update the current state from an ST3 event with improved timing"""
        ...
        # Get SimPy time if available, otherwise use event_time_s
        simpy_now = event.extra.get('simpy_now_s')
        if simpy_now is not None:
            current_time_s = simpy_now
        ...
        if event.cycle_time_ms:
            self.total_cycle_s = event.cycle_time_ms / 1000.0
            self.stage_s = max(0.8, min(8.0, self.total_cycle_s / 4.0))
        else:
            self.total_cycle_s = 10.0  # Default
            self.stage_s = self.base_stage_s
        
        # Check for busy rising edge
        if event.busy and not old_busy:
            # Normal busy rising edge
            self.cycle_start_s = current_time_s
            self.proc_active = True
            self.mid_cycle_attached = False
            print(f"ST3 busy rising edge at {current_time_s:.1f}s, cycle_time={self.total_cycle_s:.1f}s")
        
        # Handle mid-cycle attachment (busy already True but we're not active)
        elif event.busy and not self.proc_active and not self.mid_cycle_attached:
            # We missed the busy rising edge - estimate cycle start
            self.mid_cycle_attached = True
            
            # Estimate we're 75% through the cycle (in stage 3 or 4)
            self.cycle_start_s = current_time_s - 0.75 * self.total_cycle_s
            
            self.proc_active = True
            print(f"ST3 attached mid-cycle at {current_time_s:.1f}s, estimated start at {self.cycle_start_s:.1f}s")
        ...
        if event.done and not old_done:
            self.done_pulse_time = current_time_s
            self.proc_active = False
            self.active_stage_index = -1
            self.stage_progress = 0.0
            self.mid_cycle_attached = False
        ...
        # Update animation time for deterministic animations
        # Ensure animation time never goes backward
        if current_time_s > self.animation_time:
            self.animation_time = current_time_s
    ...
    def initializeGL(self):
        if PYOPENGL_AVAILABLE:
            ...
            # Initialize GLUT once (for sphere/torus primitives)
            if not self._glut_inited:
                try:
                    glutInit(sys.argv)
                    self._glut_inited = True
                    print("GLUT initialized successfully")
                except Exception as e:
                    print(f"GLUT initialization warning: {e}")
    ...
    def paintGL(self):
        ...
        try:
            ...
            traceback.print_exc()
            # Continue without crashing - clear and show error
            try:
                glClearColor(0.1, 0.1, 0.15, 1.0)
                glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT)
            except:
    ...
    def draw_scene(self):
        """Draw the enhanced 3D scene for ST3 Electronics Wiring"""
        # Draw floor grid
        self.draw_grid()
        ...
        # 1. Base table (big dark plate)
        self.draw_plate(0, -0.25, 0, 12, 10, 0.5, self.colors["base_table"])
        
        # 2. Fixture (raised block on table)
        self.draw_box(0, 0.5, 0, 3, 1, 2, self.colors["fixture"])
        
        # 3. PCB (green plate) on top of fixture
        self.draw_plate(0, 1.2, 0, 1.5, 1, 0.1, self.colors["pcb"])
        
        # 4. Wire spool reels (left side)
        spool_positions = [(-4.5, 2.5, -2.0), (-4.5, 2.5, -1.0), (-4.5, 2.5, 0.0), 
                          (-4.5, 2.5, 1.0), (-4.5, 2.5, 2.0)]
        for i, pos in enumerate(spool_positions):
            self.draw_spool_reel(pos[0], pos[1], pos[2], self.colors["spool"])
        
        # 5. Cable duct/channel from spools to PCB
        self.draw_cable_duct((-3.0, 1.8, 0), (0.0, 1.5, 0), 0.8, 0.2)
        
        # 6. Connector housings on PCB edge
        connector_positions = [(-0.5, 1.35, 0.3), (-0.2, 1.35, 0.4), (0.1, 1.35, 0.3), 
                              (0.4, 1.35, 0.4), (0.7, 1.35, 0.3)]
        for pos in connector_positions:
            self.draw_connector_housing(pos[0], pos[1], pos[2])
        
        # 7. Tie/clip points along routing path
        tie_positions = [(-2.0, 1.7, 0), (-1.0, 1.65, 0), (0.5, 1.6, 0)]
        for pos in tie_positions:
            self.draw_zip_tie(pos[0], pos[1], pos[2])
        
        # 8. Tester box with LCD panel on the right
        continuity_ok = self.current_state['extra'].get('continuity_ok')
        self.draw_tester_box(4.0, 1.0, 0, continuity_ok)
        
        # 9. Component bin (for stage 0)
        self.draw_box(-3.0, 1.0, 3.5, 1.0, 0.8, 1.0, self.colors["bin"])
        
        # ===== STRAIN RELIEF INDICATOR LIGHT =====
        strain_relief_ok = self.current_state['extra'].get('strain_relief_ok')
        strain_color = self.colors["strain_orange"]  # Default to orange (not OK)
        
        if strain_relief_ok == 1:
            # Create pulsed version of strain_green using animation_time for determinism
            pulse = 0.3 * math.sin(self.animation_time * 3) + 0.7
            base_green = self.colors["strain_green"]
            strain_color = (
                base_green[0] * pulse,
                base_green[1] * pulse,
                base_green[2] * pulse,
        ...
        elif strain_relief_ok == 0:
            strain_color = self.colors["strain_orange"]
        
        # Draw indicator light on left side of table
        glPushMatrix()
        glTranslatef(-5, 1.5, -3)
        self.draw_box(0, 0, 0, 0.3, 0.3, 0.3, strain_color)
        glPopMatrix()
        
        # ===== STAGE-BASED ANIMATIONS =====
        self.compute_active_stage(self.animation_time)
        
        if self.active_stage_index == 0:  # Load Electronics
            # Component moves from bin to PCB position
            bin_x, bin_y, bin_z = -3.0, 1.8, 3.5
            pcb_x, pcb_y, pcb_z = 0, 1.3, 0
            
            # Interpolate based on progress
            t = self.stage_progress
            x = bin_x + (pcb_x - bin_x) * t
            y = bin_y + (pcb_y - bin_y) * t
            z = bin_z + (pcb_z - bin_z) * t
            
            # Draw 3 components (resistor, capacitor, IC)
            self.draw_box(x - 0.2, y, z - 0.1, 0.15, 0.05, 0.3, self.colors["component"])  # Resistor
            self.draw_box(x, y + 0.1, z, 0.2, 0.3, 0.2, self.colors["component"])  # IC
            self.draw_box(x + 0.2, y, z + 0.1, 0.1, 0.2, 0.1, self.colors["component"])  # Capacitor
            
        elif self.active_stage_index == 1:  # Route Wires
            # Wire harness grows from spools to PCB
            self.draw_wire_harness(self.stage_progress)
            
        elif self.active_stage_index == 2:  # Connect/Crimp
            # Crimp tool at connector position
            self.draw_crimp_tool(0.2, 1.5, 0.3, self.stage_progress)
            
        elif self.active_stage_index == 3:  # Continuity Test
            # Two probes touching PCB pads
            self.draw_probe(-0.2, 1.8, 0.2, 0.0)
            self.draw_probe(0.2, 1.8, 0.2, math.pi)
            
            # LCD shows result when test is complete
            if self.stage_progress > 0.6:
                continuity_ok = self.current_state['extra'].get('continuity_ok')
                if continuity_ok is not None:
                    # Redraw tester with updated status
                    self.draw_tester_box(4.0, 1.0, 0, continuity_ok)
    ...
    def draw_overlay(self, painter):
        """Draw overlay text with ST3 system information"""
        # Setup font
        ...
        if self.active_stage_index >= 0 and self.active_stage_index < 4:
            ...
            # Add PASS/FAIL result based on continuity_ok
            if self.active_stage_index == 3 and self.stage_progress > 0.6:
                continuity_ok = self.current_state['extra'].get('continuity_ok')
                if continuity_ok is not None:
                    if continuity_ok == 1:
                        active_stage += " (PASS)"
        ...
        # ST3-specific quality signals
        strain_relief_ok = self.current_state['extra'].get('strain_relief_ok', 'N/A')
        continuity_ok = self.current_state['extra'].get('continuity_ok', 'N/A')
        
        # Determine PASS/FAIL colors
        if continuity_ok == 1:
            continuity_text = "Continuity: PASS"
            continuity_color = QColor(0, 255, 0)
        elif continuity_ok == 0:
            continuity_text = "Continuity: FAIL"
            continuity_color = QColor(255, 0, 0)
        else:
            continuity_text = f"Continuity: {continuity_ok}"
            continuity_color = QColor(200, 200, 200)
        
        if strain_relief_ok == 1:
            strain_text = "Strain Relief: OK"
            strain_color = QColor(0, 255, 0)
        elif strain_relief_ok == 0:
            strain_text = "Strain Relief: NOT OK"
            strain_color = QColor(255, 165, 0)  # Orange warning
        else:
            strain_text = f"Strain Relief: {strain_relief_ok}"
            strain_color = QColor(200, 200, 200)
        
        quality_text = f"{continuity_text} | {strain_text}"
        ...
        # Time model info
        time_model_text = f"Cycle Start: {self.cycle_start_s:.1f}s"
        ...
        texts = [mode_text, state_text, info_text, stage_text, 
                continuity_text, strain_text, cycle_text, stage_dur_text, time_model_text]
        
        colors = [None, None, None, None, 
                 continuity_color, strain_color, None, None, None]
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("ST3 Electronics Wiring Visualizer")
        ...
        # For deterministic LIVE mode timing
        self._last_visual_time_s = time.time()
        self._last_simpy_time_s = None
    ...
    def get_visual_time_s(self):
        ...
        if self.is_live_mode:
            ...
            if self.current_event:
                ...
                if simpy_now is not None:
                    # Ensure monotonic increase in LIVE mode
                    if self._last_simpy_time_s is None or simpy_now > self._last_simpy_time_s:
                        self._last_simpy_time_s = simpy_now
                    return self._last_simpy_time_s
            
            # Fallback to wall time for LIVE mode, ensure monotonic
            current_wall_time = time.time()
            if current_wall_time > self._last_visual_time_s:
                self._last_visual_time_s = current_wall_time
            return self._last_visual_time_s
        else:
    ...
    def init_ui(self):
        ...
        self.log_info_label = QLabel("Searching for ST3 Electronics Wiring log...")
        ...
        self.gl_widget = ST3OpenGLWidget()
        ...
        # State labels
        state_group = QGroupBox("ST3 State")
        ...
        # Stage info
        stage_group = QGroupBox("Wiring Process Status")
        ...
        # ST3 Quality Signals
        quality_group = QGroupBox("ST3 Quality Signals")
        quality_layout = QVBoxLayout()
        
        self.continuity_label = QLabel("Continuity: N/A")
        self.strain_relief_label = QLabel("Strain Relief: N/A")
        
        for label in [self.continuity_label, self.strain_relief_label]:
            label.setStyleSheet("font-family: monospace; padding: 2px;")
            quality_layout.addWidget(label)
        
        quality_group.setLayout(quality_layout)
        status_panel.addWidget(quality_group)
    ...
    def load_live_snapshot(self) -> Tuple[Optional[ST3Event], int]:
    ...
    def discover_log(self):
        """Discover and load ST3 log file"""
        # Stop current worker
        self._stop_tail_worker()
        
        # Find log
        self.log_path = LogDiscoverer.find_st3_log()
        
        if self.log_path:
            base_name = os.path.basename(self.log_path)
            self.log_info_label.setText(f"ST3 Log: {base_name}")
            ...
            self.log_info_label.setText("No ST3 log found")
            self.log_path = None
            self.replay_events = []
    ...
    def load_replay_data(self):
        ...
        try:
            ...
            print(f"Loaded {len(self.replay_events)} ST3 events for replay")
    ...
    def update_states_from_replay(self):
        ...
        if current_event:
            # Get event time for update_state - use SimPy time if available
            simpy_now = current_event.extra.get('simpy_now_s')
            if simpy_now is not None:
                event_time_s = simpy_now
            else:
                # Fallback to VSI time
                event_time_s = (current_time - self.replay_min_time) / 1e9
    ...
    def process_new_event(self, event: ST3Event):
    ...
    def update_display(self, event: ST3Event, from_snapshot: bool = False):
        """Update all displays from ST3 event"""
        ...
        visual_time_s = self.get_visual_time_s()
        now_text = f"Visual Time: {visual_time_s:.1f}s"
        ...
        # Update ST3 quality signals
        strain_relief_ok = event.extra.get('strain_relief_ok', 'N/A')
        continuity_ok = event.extra.get('continuity_ok', 'N/A')
        
        # Update continuity label with color
        if continuity_ok == 1:
            self.continuity_label.setText("Continuity: PASS")
            self.continuity_label.setStyleSheet("font-family: monospace; padding: 2px; color: #00FF00;")
        elif continuity_ok == 0:
            self.continuity_label.setText("Continuity: FAIL")
            self.continuity_label.setStyleSheet("font-family: monospace; padding: 2px; color: #FF0000;")
        else:
            self.continuity_label.setText(f"Continuity: {continuity_ok}")
            self.continuity_label.setStyleSheet("font-family: monospace; padding: 2px; color: #CCCCCC;")
        
        # Update strain relief label with color
        if strain_relief_ok == 1:
            self.strain_relief_label.setText("Strain Relief: OK")
            self.strain_relief_label.setStyleSheet("font-family: monospace; padding: 2px; color: #00FF00;")
        elif strain_relief_ok == 0:
            self.strain_relief_label.setText("Strain Relief: NOT OK")
            self.strain_relief_label.setStyleSheet("font-family: monospace; padding: 2px; color: #FFA500;")  # Orange warning
        else:
            self.strain_relief_label.setText(f"Strain Relief: {strain_relief_ok}")
            self.strain_relief_label.setStyleSheet("font-family: monospace; padding: 2px; color: #CCCCCC;")
...
def main():
    ...
    print("Running ST3 Electronics Wiring visualizer from:", os.path.abspath(__file__))

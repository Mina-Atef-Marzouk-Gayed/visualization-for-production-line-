...
class ST3LogParser:
    ...
    def reset(self):
        ...
        # Time advanced with delta (ST3 specific)
...
class ST3OpenGLWidget(QOpenGLWidget):
    ...
    def __init__(self, parent=None):
        ...
        # ST3 stage animation parameters
    ...
    def compute_active_stage(self, visual_time_s=None):
        """Compute which ST3 stage is active and its progress based on visual_time_s"""
        ...
        # If busy but timer ended, stay in stage 4 (Continuity Test) with looping animation
        if visual_time_s >= self.proc_end:
            if self.current_state['busy']:
                # Stay in stage 4 with looping tester
                self.active_stage_index = 3
                # Loop tester every 2 seconds (using visual_time_s for determinism)
    ...
    def draw_overlay(self, painter):
        ...
        texts = [mode_text, state_text, info_text, stage_text, 
                continuity_text, strain_text, cycle_text, stage_dur_text]
        
        colors = [None, None, None, None, 
                 continuity_color, strain_color, None, None]
    ...
    def draw_scene(self):
        """Draw the 3D scene for ST3 Electronics Wiring"""
        # Draw floor grid
        self.draw_grid()
        ...
        # 4. Wire anchor points markers (small cubes)
        # Left anchor
        self.draw_box(-1.5, 0.8, 0.8, 0.1, 0.1, 0.1, self.colors["anchor_point"])
        # Right anchor
        self.draw_box(1.5, 0.8, 0.8, 0.1, 0.1, 0.1, self.colors["anchor_point"])
        # Front anchor
        self.draw_box(0, 0.8, 1.5, 0.1, 0.1, 0.1, self.colors["anchor_point"])
        ...
        # If None, keep the default orange
        ...
        if self.active_stage_index == 0:  # Load Electronics
            # Component cube moves from bin position to PCB position
            bin_x, bin_y, bin_z = -5, 2.0, 3.0
            ...
            # Draw component cube
            self.draw_box(x, y, z, 0.4, 0.4, 0.4, self.colors["component"])
            
        elif self.active_stage_index == 1:  # Route Wires
            # Wire polyline gradually appears
            # Define wire path from anchor to PCB
            wire_points = [
                (-1.5, 0.8, 0.8),  # Left anchor
                (-1.0, 1.0, 0.5),
                (-0.5, 1.1, 0.2),
                (0, 1.25, 0),      # PCB center
                (0.5, 1.1, -0.2),
                (1.0, 1.0, -0.5),
                (1.5, 0.8, -0.8)   # Right anchor
            ]
            
            # Draw wire with partial progress
            self.draw_wire_polyline(wire_points, self.stage_progress, self.colors["wire"])
            
        elif self.active_stage_index == 2:  # Connect/Crimp
            # Two crimp jaws move toward each other
            # Start positions
            left_jaw_x = -1.0 - (1 - self.stage_progress) * 0.5
            right_jaw_x = 1.0 + (1 - self.stage_progress) * 0.5
            
            # Draw left crimp jaw
            self.draw_box(left_jaw_x, 1.0, 0, 0.2, 0.3, 0.1, self.colors["crimp_jaw"])
            # Draw right crimp jaw
            self.draw_box(right_jaw_x, 1.0, 0, 0.2, 0.3, 0.1, self.colors["crimp_jaw"])
            
        elif self.active_stage_index == 3:  # Continuity Test
            # Probe moving down/up (sin wave) - using animation_time for determinism
            probe_y = 2.0 - math.sin(self.animation_time * 3) * 0.5
            
            # Draw probe
            self.draw_box(0, probe_y, 0, 0.1, 0.5, 0.1, self.colors["tester"])
            
            # PASS/FAIL panel above PCB
            continuity_ok = self.current_state['extra'].get('continuity_ok')
            if continuity_ok == 1:
                ...
                panel_color = self.colors["neutral_gray"]  # Neutral when unknown
            
            # Only show panel when test is complete (progress > 0.7)
            if self.stage_progress > 0.7:
                # Panel position with slight hover animation
                panel_y = 2.5 + 0.1 * math.sin(self.animation_time * 2)
                self.draw_plate(0, panel_y, 0, 1.2, 0.8, 0.05, panel_color)
    ...
    def update_state(self, event: ST3Event, mode: str = "LIVE", event_time_s: float = None):
        """Update the current state from an ST3 event"""
        ...
        # Handle case where visualizer attaches mid-cycle (busy already True but not active)
            ...
            # Estimate we're in stage 3 or 4 (Connect/Crimp or Continuity Test) since we're already busy
            ...
            print(f"ST3 attached mid-cycle: starting animation at estimated stage 3/4")

...
- FIXED KPI Calculations: Throughput, Utilization, Station-Specific Charts
...
Logs should be placed in current directory or subdirectories with names:
    ...
    check.ST1_*.log … check.ST6_*.log

Author: DeepSeek (Senior Qt/Python Engineer)
Version: 1.4 - Fixed KPI Math + Station Charts
"""
...
# ===== STATION-SPECIFIC CHART CONFIGURATION =====
@dataclass
class ChartDefinition:
    """Definition for a station-specific chart"""
    title: str
    chart_type: str  # 'line', 'bar', 'pie', 'scatter', 'multiline'
    data_field: str  # Field name in extra_fields to extract data from
    y_label: str = ""
    x_label: str = "Time"
    color: str = None  # Optional specific color
    aggregation: str = "last"  # 'last', 'mean', 'sum', 'delta'
    show_trend: bool = False  # Add trend line if possible
    
    def get_color(self, station_id: str = None):
        """Get color for this chart"""
        if self.color:
            return self.color
        elif station_id and station_id in STATIONS:
            return STATIONS[station_id]['color']
        return "#4FC3F7"

# Station-specific chart configurations
STATION_CHARTS = {
    "ST1": [
        ChartDefinition(
            title="Parts Loaded Over Time",
            chart_type="line",
            data_field="parts_loaded",
            y_label="Parts Loaded",
            color="#4FC3F7"
        ),
        ChartDefinition(
            title="Load Cycle Times",
            chart_type="line", 
            data_field="load_cycle_time",
            y_label="Cycle Time (ms)",
            color="#29B6F6",
            show_trend=True
        ),
    ],
    "ST2": [
        ChartDefinition(
            title="Assembly Steps Completed",
            chart_type="line",
            data_field="assembly_steps",
            y_label="Steps",
            color="#29B6F6"
        ),
        ChartDefinition(
            title="Torque Measurements",
            chart_type="scatter",
            data_field="torque_value",
            y_label="Torque (Nm)",
            color="#0288D1"
        ),
    ],
    "ST3": [
        ChartDefinition(
            title="Test Results",
            chart_type="pie",
            data_field="test_result",  # Will count pass/fail
            y_label="",
            color="#0288D1"
        ),
        ChartDefinition(
            title="Test Voltage Trend",
            chart_type="line",
            data_field="test_voltage",
            y_label="Voltage (V)",
            color="#0277BD"
        ),
    ],
    "ST4": [
        ChartDefinition(
            ...
            chart_type="line",
            data_field="cal_offset",
            y_label="Offset (units)",
            color="#0277BD"
        ),
        ChartDefinition(
            title="Calibration Retries",
            chart_type="bar",
            data_field="cal_retries",
            y_label="Retry Count",
            color="#01579B"
        ),
    ],
    "ST5": [
        ChartDefinition(
            title="Accept/Reject Ratio",
            chart_type="pie",
            data_field="last_accept",  # 1=accept, 0=reject
            y_label="",
            color="#01579B"
        ),
        ChartDefinition(
    ...
    "ST6": [
        ChartDefinition(
            ...
            chart_type="line",
            data_field="packages_completed",
            y_label="Packages",
            color="#039BE5",
            show_trend=True
        ),
        ChartDefinition(
            title="Packaging Cycle Times",
            chart_type="line",
            data_field="pack_cycle_time",
            y_label="Cycle Time (ms)",
            color="#4FC3F7"
        ),
    ]
...
# Default fallback charts if station-specific fields not found
FALLBACK_CHARTS = [
    ChartDefinition(
        title="Cycle Time Trend",
        chart_type="line",
        data_field="cycle_time",
        y_label="Cycle Time (ms)"
    ),
    ChartDefinition(
        title="Station State Distribution",
        chart_type="bar",
        data_field="state_changes",
        y_label="Count"
    )
]
...
class KpiSnapshot:
    ...
    # New: For tracking station-specific metrics
...
# ===== PARSER MODULE WITH IMPROVED TIMESTAMP HANDLING =====
class StationLogParser:
    """Carry-forward parser for station logs with FIXED monotonic timestamps"""
    ...
    def reset(self):
        ...
        self.last_emitted_time_ns = 0  # Track last emitted timestamp for monotonic guarantee
        ...
        # Generic key-value pattern (more robust)
        self.key_value_pattern = re.compile(r"(\w+)[_\s]*[:=]\s*([-\w.]+(?:[eE][+-]?\d+)?)")
        
        # State patterns
        self.state_patterns = {
            "ready": re.compile(r"\bready\b[_\s]*[:=]\s*([01]|true|false|yes|no)", re.IGNORECASE),
            "busy": re.compile(r"\bbusy\b[_\s]*[:=]\s*([01]|true|false|yes|no)", re.IGNORECASE),
            "done": re.compile(r"\bdone\b[_\s]*[:=]\s*([01]|true|false|yes|no)", re.IGNORECASE),
            "fault": re.compile(r"\bfault\b[_\s]*[:=]\s*([01]|true|false|yes|no)", re.IGNORECASE),
        }
        
        # Counter patterns for completion/reject tracking
        self.completion_patterns = [
            re.compile(r"\bpackages?[_\s]*completed\b[_\s]*[:=]\s*(\d+)", re.IGNORECASE),
            re.compile(r"\bcompleted[_\s]*parts?\b[_\s]*[:=]\s*(\d+)", re.IGNORECASE),
            re.compile(r"\btotal[_\s]*completed\b[_\s]*[:=]\s*(\d+)", re.IGNORECASE),
        ]
        
        self.reject_patterns = [
            re.compile(r"\brejects?\b[_\s]*[:=]\s*(\d+)", re.IGNORECASE),
            re.compile(r"\bscrapped\b[_\s]*[:=]\s*(\d+)", re.IGNORECASE),
            re.compile(r"\brejected[_\s]*count\b[_\s]*[:=]\s*(\d+)", re.IGNORECASE),
            re.compile(r"\btotal[_\s]*rejects\b[_\s]*[:=]\s*(\d+)", re.IGNORECASE),
        ]
    ...
    def seed_from_event(self, event: StationEvent):
        ...
        self.last_emitted_time_ns = event.t_ns
    ...
    def parse_line(self, line: str) -> Optional[StationEvent]:
        ...
        # Determine timestamp with MONOTONIC guarantee
        ...
        # Check for completion counters (ST6 specific)
        if self.station_id == "ST6":
            for pattern in self.completion_patterns:
                match = pattern.search(line)
                if match:
                    try:
                        completed = int(match.group(1))
                        current_line_extras['packages_completed'] = completed
                        current_line_extras['_counter_completion'] = completed  # Special marker
                        had_signal = True
                    except ValueError:
                        pass
        
        # Check for reject counters (ST5 specific)
        if self.station_id == "ST5":
            for pattern in self.reject_patterns:
                match = pattern.search(line)
                if match:
                    try:
                        rejected = int(match.group(1))
                        current_line_extras['rejects'] = rejected
                        current_line_extras['_counter_reject'] = rejected  # Special marker
                        had_signal = True
                    except ValueError:
                        pass
        ...
        if had_signal:
            ...
            self.last_emitted_time_ns = timestamp
    ...
    def _get_monotonic_timestamp(self) -> int:
        """Get a monotonic increasing timestamp for events"""
        # Prefer VSI time if available
        if self.last_vsi_time_ns is not None:
            base_time = self.last_vsi_time_ns
        else:
            # Use synthetic time
            self.synthetic_time_ns += 10_000_000  # 10ms increment
            base_time = self.synthetic_time_ns
        
        # Ensure monotonic increase
        if base_time <= self.last_emitted_time_ns:
            # Add a small delta (1ms) to keep time moving
            base_time = self.last_emitted_time_ns + 1_000_000
...
# ===== KPI CALCULATOR WITH FIXED MATH =====
class KpiCalculator(QObject):
    """Calculates KPIs from station events with FIXED throughput/utilization"""
    ...
    def reset(self):
        ...
        with self.lock:
            self.station_states = {st_id: {
                ...
                # New: Counter tracking for delta calculations
                'last_completion_counter': 0,
                'last_reject_counter': 0,
            ...
            self.last_throughput_calc_time = 0
    
    def process_event(self, event: StationEvent):
        """Process a new event and update KPIs with FIXED counting"""
        with self.lock:
            ...
            # Update start time if first event (use event time, not system time)
            if self.start_time_ns == 0:
                self.start_time_ns = current_time_ns
                self.last_throughput_calc_time = current_time_ns
            ...
            # Update debug info
            station['debug_info']['last_parsed_time'] = current_time_ns
            station['debug_info']['last_state'] = self._get_state_name(event)
            station['debug_info']['last_extras_keys'] = list(event.extra.keys())
            ...
            # Update extra fields and track counters
            if event.extra:
                station['extra_fields'].update(event.extra)
                
                # ST5: Track accept state
                if station_id == "ST5":
                    if 'last_accept' in event.extra:
                        station['last_accept_state'] = 1 if event.extra['last_accept'] else 0
                    station['debug_info']['last_counter_value'] = station['last_accept_state']
                
                # ST6: Track completion via counter (more reliable than done pulses)
                if station_id == "ST6" and '_counter_completion' in event.extra:
                    current_counter = event.extra['_counter_completion']
                    last_counter = station['last_completion_counter']
                    
                    if current_counter > last_counter:
                        delta = current_counter - last_counter
                        station['cumulative_completed'] += delta
                        self.total_completed += delta
                        station['done_count'] += delta
                        
                        # Store for throughput smoothing
                        self.completion_history.append((current_time_ns, delta))
                    
                    station['last_completion_counter'] = current_counter
                    station['debug_info']['last_counter_value'] = current_counter
                
                # ST5: Track rejects via counter
                if station_id == "ST5" and '_counter_reject' in event.extra:
                    current_counter = event.extra['_counter_reject']
                    last_counter = station['last_reject_counter']
                    
                    if current_counter > last_counter:
                        delta = current_counter - last_counter
                        station['cumulative_rejected'] += delta
                        self.total_rejected += delta
                        station['reject_count'] += delta
                    
                    station['last_reject_counter'] = current_counter
            
            # FIXED: Track busy time - handle initial busy state from snapshot
            ...
            # FALLBACK: Track completion via ST6 DONE pulses (if no counter)
            if event.done and station_id == "ST6" and '_counter_completion' not in event.extra:
                station['done_count'] += 1
                self.total_completed += 1
                self.completion_history.append((current_time_ns, 1))
            
            # FALLBACK: Track reject via ST5 done + last_accept==0 (if no counter)
            if (station_id == "ST5" and event.done and 
                station['last_accept_state'] == 0 and
                station['last_done_pulse_time'] == current_time_ns and
                '_counter_reject' not in event.extra):
    ...
    def _get_state_name(self, event: StationEvent) -> str:
    ...
    def calculate_kpis(self, current_time_ns: int):
        """Calculate all KPIs at current time with FIXED math"""
        with self.lock:
            if self.start_time_ns == 0:
                return
            ...
            # FIX: Prevent division by zero and handle early time
            if elapsed_time_ns < 1_000_000:  # Less than 1ms
                elapsed_time_ns = 1_000_000
            
            elapsed_time_hours = elapsed_time_ns / (1e9 * 3600)
            elapsed_time_minutes = elapsed_time_ns / (1e9 * 60)
            
            # Calculate throughput with IMPROVED smoothing
            throughput_ph = 0
            throughput_pm = 0
            
            if self.total_completed > 0:
                # Calculate based on total completed over elapsed time
                ...
                # Also calculate short-term throughput from recent completions
                if len(self.completion_history) >= 2:
                    recent_time_window = current_time_ns - self.completion_history[0][0]
                    if recent_time_window > 1e9:  # At least 1 second of data
                        recent_completions = sum(delta for _, delta in self.completion_history)
                        recent_hours = recent_time_window / (1e9 * 3600)
                        if recent_hours > 0:
                            # Blend short-term with long-term
                            short_term_throughput = recent_completions / recent_hours
                            throughput_ph = 0.7 * throughput_ph + 0.3 * short_term_throughput
            ...
            # FIXED: Calculate utilization and availability
            total_utilization = 0
            ...
            station_metrics = {}  # For station-specific metrics
            
            done_latch_ns = DONE_LATCH_MS * 1_000_000
            
            for station_id, station in self.station_states.items():
                # Station state with DONE latching
                if station['fault']:
                ...
                # FIXED: Utilization (percentage of time busy)
                if elapsed_time_ns > 0:
                    current_busy_time = station['total_busy_time']
                    
                    # If currently busy, add time since busy_start_time
                    if station['busy'] and station['busy_start_time'] > 0:
                        current_busy_time += (current_time_ns - station['busy_start_time'])
                    # If busy at start but no busy_start_time set (from snapshot), estimate
                    elif station['busy'] and station['busy_start_time'] == 0:
                        current_busy_time += elapsed_time_ns * 0.5  # Estimate 50% busy
                    
                    utilization = (current_busy_time / elapsed_time_ns) * 100
                    station_utilization[station_id] = min(100, max(0, utilization))
                    total_utilization += utilization
                
                # FIXED: Availability (percentage of time not in fault)
                if elapsed_time_ns > 0:
                    # Assume 100% and subtract fault time if we track it
                    # For now, just set to 100% - can be enhanced with fault time tracking
                    availability = 100
                ...
                # Station-specific metrics for charts
                station_metrics[station_id] = {
                    'cumulative_completed': station.get('cumulative_completed', 0),
                    'cumulative_rejected': station.get('cumulative_rejected', 0),
                    'last_accept_state': station.get('last_accept_state', 1),
                    'debug_info': station.get('debug_info', {}),
                    # Extract numeric fields for charting
                    'numeric_fields': {k: v for k, v in station['extra_fields'].items() 
...
# ===== CHART MANAGER WITH STATION-SPECIFIC CHARTS =====
class ChartManager:
    """Manages chart generation and updates for all backends with station-specific charts"""
    ...
    @staticmethod
    def get_station_charts(station_id: str, kpi_history: deque) -> List[Dict]:
        """Get chart definitions and data for a specific station"""
        charts = []
        
        # Get station-specific chart definitions
        chart_defs = STATION_CHARTS.get(station_id, FALLBACK_CHARTS)
        
        for chart_def in chart_defs:
            chart_data = ChartManager._extract_chart_data(chart_def, station_id, kpi_history)
            if chart_data:
        ...
        if len(charts) < 2:
            # Add fallback charts
            for chart_def in FALLBACK_CHARTS:
                if len(charts) >= 2:
                    break
                chart_data = ChartManager._extract_chart_data(chart_def, station_id, kpi_history)
                if chart_data:
    ...
    @staticmethod
    def _extract_chart_data(chart_def: ChartDefinition, station_id: str, kpi_history: deque) -> Optional[Dict]:
        """Extract data for a specific chart definition"""
        if not kpi_history:
            return None
        ...
        try:
            ...
            if chart_def.chart_type == 'line':
                return ChartManager._create_line_chart_data(chart_def, station_id, kpi_history)
            elif chart_def.chart_type == 'bar':
                return ChartManager._create_bar_chart_data(chart_def, station_id, kpi_history)
            elif chart_def.chart_type == 'pie':
                return ChartManager._create_pie_chart_data(chart_def, station_id, kpi_history)
            elif chart_def.chart_type == 'scatter':
                return ChartManager._create_scatter_chart_data(chart_def, station_id, kpi_history)
            elif chart_def.chart_type == 'multiline':
                return ChartManager._create_multiline_chart_data(chart_def, station_id, kpi_history)
        except Exception as e:
            print(f"Error extracting chart data for {station_id} - {chart_def.title}: {e}")
        
        return None
    
    @staticmethod
    def _create_line_chart_data(chart_def: ChartDefinition, station_id: str, kpi_history: deque) -> Dict:
        """Create line chart data from history"""
        times = []
        values = []
        
        for snapshot in kpi_history:
            times.append(snapshot.timestamp_s)
            
            # Try to get value from station metrics
            station_metrics = snapshot.station_metrics.get(station_id, {})
            numeric_fields = station_metrics.get('numeric_fields', {})
            
            value = None
            
            # Try different field names
            if chart_def.data_field in numeric_fields:
                value = numeric_fields[chart_def.data_field]
            elif chart_def.data_field in snapshot.station_extra_fields.get(station_id, {}):
                val = snapshot.station_extra_fields[station_id][chart_def.data_field]
                if isinstance(val, (int, float)):
                    value = val
            elif chart_def.data_field == 'cycle_time':
                value = snapshot.avg_cycle_times.get(station_id, 0.0)
            
            values.append(value if value is not None else 0.0)
        
        # Filter out None values and corresponding times
        valid_data = [(t, v) for t, v in zip(times, values) if v is not None]
        if not valid_data:
            valid_data = [(t, 0.0) for t in times]
        
        times, values = zip(*valid_data) if valid_data else ([], [])
        ...
        if CHART_BACKEND == "plotly_webengine":
            trace = {
                ...
                'line': {'color': chart_def.get_color(station_id), 'width': 2},
            }
            
            layout = {
                'title': chart_def.title,
                'xaxis': {'title': chart_def.x_label},
                'yaxis': {'title': chart_def.y_label},
                'showlegend': True,
                'height': 400,
                **ChartManager.DARK_THEME
            }
            
            return {'type': 'plotly', 'data': [trace], 'layout': layout}
        else:
            return {
                'type': 'line',
                'x': list(times),
                'y': list(values),
                'title': chart_def.title,
                'xlabel': chart_def.x_label,
                'ylabel': chart_def.y_label,
                'color': chart_def.get_color(station_id)
            }
    
    @staticmethod
    def _create_bar_chart_data(chart_def: ChartDefinition, station_id: str, kpi_history: deque) -> Dict:
        """Create bar chart data"""
        if not kpi_history:
            return {}
        
        latest = kpi_history[-1]
        station_metrics = latest.station_metrics.get(station_id, {})
        numeric_fields = station_metrics.get('numeric_fields', {})
        
        # For bar chart, show latest values of multiple fields
        categories = []
        values = []
        
        # Show top 5 numeric fields
        for field_name, value in list(numeric_fields.items())[:5]:
            if isinstance(value, (int, float)):
                categories.append(field_name[:15])  # Truncate long names
        ...
        if CHART_BACKEND == "plotly_webengine":
            trace = {
                ...
                'type': 'bar',
                'name': chart_def.title,
                'marker': {'color': chart_def.get_color(station_id)},
            }
            ...
            return {
                ...
                'xlabel': chart_def.x_label,
                'ylabel': chart_def.y_label,
                'colors': [chart_def.get_color(station_id)] * len(categories)
            }
    
    @staticmethod
    def _create_pie_chart_data(chart_def: ChartDefinition, station_id: str, kpi_history: deque) -> Dict:
        ...
        if chart_def.data_field == 'last_accept':
            station_metrics = latest.station_metrics.get(station_id, {})
            accept_state = station_metrics.get('last_accept_state', 1)
            
            labels = ['Accept', 'Reject']
            values = [100 if accept_state == 1 else 0, 100 if accept_state == 0 else 0]
        else:
            # Generic pie chart from latest numeric fields
            station_metrics = latest.station_metrics.get(station_id, {})
            numeric_fields = station_metrics.get('numeric_fields', {})
            ...
            for field_name, value in list(numeric_fields.items())[:4]:
                if isinstance(value, (int, float)) and value > 0:
                    labels.append(field_name[:10])
                    values.append(float(value))
        ...
        if CHART_BACKEND == "plotly_webengine":
            trace = {
                ...
                'type': 'pie',
                'name': chart_def.title,
                'hole': 0.4 if len(labels) > 1 else 0,
                'marker': {'colors': [chart_def.get_color(station_id)] * len(labels)},
            }
            ...
            return {
                ...
                'values': values,
                'title': chart_def.title,
                'colors': [chart_def.get_color(station_id)] * len(labels)
            }
    
    @staticmethod
    def _create_scatter_chart_data(chart_def: ChartDefinition, station_id: str, kpi_history: deque) -> Dict:
        ...
        for snapshot in kpi_history:
            ...
            if chart_def.data_field in numeric_fields:
                times.append(snapshot.timestamp_s)
                values.append(float(numeric_fields[chart_def.data_field]))
    ...
    @staticmethod
    def _create_multiline_chart_data(chart_def: ChartDefinition, station_id: str, kpi_history: deque) -> Dict:
        """Create multi-line chart data (for comparing multiple stations)"""
        if not kpi_history:
            return {}
        
        # For ALL view - show all stations
        all_station_data = {}
        
        for snapshot in kpi_history:
            for st_id in STATIONS.keys():
                if st_id not in all_station_data:
                    all_station_data[st_id] = {'times': [], 'values': []}
                
                # Get value based on chart type
                if chart_def.data_field == 'cycle_time':
                    value = snapshot.avg_cycle_times.get(st_id, 0.0)
                elif chart_def.data_field == 'utilization':
                    value = snapshot.station_utilization.get(st_id, 0.0)
                else:
                    station_metrics = snapshot.station_metrics.get(st_id, {})
                    numeric_fields = station_metrics.get('numeric_fields', {})
                    value = numeric_fields.get(chart_def.data_field, 0.0)
                
                all_station_data[st_id]['times'].append(snapshot.timestamp_s)
                all_station_data[st_id]['values'].append(value)
        
        if CHART_BACKEND == "plotly_webengine":
            traces = []
            for st_id, data in all_station_data.items():
                if data['times']:
                    color = STATIONS.get(st_id, {}).get('color', '#888888')
                    trace = {
                        'x': data['times'],
                        'y': data['values'],
                        'type': 'scatter',
            ...
            return {'type': 'plotly', 'data': traces, 'layout': layout}
        else:
            series = []
            for st_id, data in all_station_data.items():
                if data['times']:
    ...
    @staticmethod
    def create_wip_chart_data(time_data, wip_data):
        """Create WIP over time chart data"""
        try:
            if CHART_BACKEND == "plotly_webengine":
                trace = {
                    'x': time_data,
            ...
            print(f"Error creating WIP chart: {e}")
            return [], {}
    
    @staticmethod
    def create_completed_rejects_chart_data(time_data, completed_data, rejects_data):
        """Create completed vs rejects over time chart data"""
        try:
            if CHART_BACKEND == "plotly_webengine":
                trace1 = {
                    'x': time_data,
                ...
                layout = {
                    'title': 'Completed vs Rejects Over Time',
                    'xaxis': {'title': 'Time'},
                    'yaxis': {'title': 'Count'},
                    'showlegend': True,
                    'height': 400,
                ...
                return {
                    'type': 'multiline',
                    'series': [
                        {'name': 'Completed', 'x': time_data, 'y': completed_data, 'color': '#4CAF50'},
                        {'name': 'Rejects', 'x': time_data, 'y': rejects_data, 'color': '#F44336'}
                    ],
                    'title': 'Completed vs Rejects Over Time',
                    'xlabel': 'Time',
                    'ylabel': 'Count'
            ...
            print(f"Error creating completed/rejects chart: {e}")
            return [], {}
...
if WEBENGINE_AVAILABLE:
    class PlotlyWidget(ChartWidget):
        ...
        def update_chart(self, data, layout):
            ...
            try:
                # Check if data is in the new station-specific format
                if isinstance(data, dict) and data.get('type') == 'plotly':
                    # New format: data already prepared for plotly
                    data_json = json.dumps(data['data'], default=self._json_serializer)
                    layout_json = json.dumps(data['layout'], default=self._json_serializer)
                else:
                    # Old format: data and layout separate
...
if QTCHARTS_AVAILABLE:
    class QtChartsWidget(ChartWidget):
        ...
        def update_chart(self, data, layout=None):
            ...
            try:
                ...
                if chart_type == 'line':
                    ...
                    self._update_scatter_chart(data)
        ...
        def _update_pie_chart(self, data):
            """Update pie chart"""
            series = QPieSeries()
            if 'hole' in data and data.get('hole', 0) > 0:
                series.setHoleSize(data['hole'])  # Donut chart
        ...
        def _update_scatter_chart(self, data):
            ...
            series.setBorderColor(color.darker())
...
if MATPLOTLIB_AVAILABLE:
    class MatplotlibWidget(ChartWidget):
        ...
        def _update_scatter_chart(self, data):
            ...
            self.ax.scatter(x_data, y_data, color=color, s=50, alpha=0.7)
            
            # Add trend line if requested
            if len(x_data) > 1:
                z = np.polyfit(x_data, y_data, 1)
                p = np.poly1d(z)
                self.ax.plot(x_data, p(x_data), color=color, linestyle='--', alpha=0.5)
...
class SimpleChartWidget(ChartWidget):
    ...
    def paintEvent(self, event):
        ...
        if self.chart_type == 'line':
            ...
            self._draw_scatter_chart(painter)
        else:
            # Draw title
    ...
    def _draw_scatter_chart(self, painter):
        """Draw a simple scatter chart"""
        # Draw title
        title = self.chart_data.get('title', 'Scatter Chart')
        painter.setPen(QPen(CHART_COLORS['text']))
        painter.setFont(QFont("Segoe UI", 12, QFont.Bold))
        painter.drawText(10, 25, title)
...
class DebugPanel(QWidget):
    """Collapsible debug panel to show parsing status"""
    
    def __init__(self, kpi_calculator: KpiCalculator, parent=None):
        super().__init__(parent)
    ...
    def init_ui(self):
        ...
        # Header
        header = QFrame()
        header.setStyleSheet("background-color: #2a2a3a; border-radius: 5px;")
        header_layout = QHBoxLayout(header)
        
        title = QLabel("📊 Debug Panel")
        title.setStyleSheet("color: #4FC3F7; font-weight: bold; font-size: 14px;")
        ...
        self.debug_table.setColumnCount(6)
        self.debug_table.setHorizontalHeaderLabels(["Station", "Last Time", "State", "Keys", "Counter", "Busy"])
        ...
        self.debug_table.setMaximumHeight(200)
        
        self.content_layout.addWidget(self.debug_table)
        
        # Status labels
        status_layout = QHBoxLayout()
        
        self.kpi_status = QLabel("KPI: Idle")
        self.kpi_status.setStyleSheet("color: #FF9800;")
        status_layout.addWidget(self.kpi_status)
        
        self.event_status = QLabel("Events: 0")
        self.event_status.setStyleSheet("color: #4CAF50;")
        status_layout.addWidget(self.event_status)
    ...
    def update_debug_info(self, kpi_snapshot: KpiSnapshot):
        """Update debug panel with latest info"""
        try:
            with self.kpi_calculator.lock:
                station_states = self.kpi_calculator.station_states
                
                self.debug_table.setRowCount(len(station_states))
                
                for row, (station_id, station) in enumerate(station_states.items()):
                    debug_info = station.get('debug_info', {})
                    
                    # Station ID
                    self.debug_table.setItem(row, 0, QTableWidgetItem(station_id))
                    
                    # Last parsed time
                    last_time = debug_info.get('last_parsed_time', 0)
                    if last_time > 0:
                        time_str = f"{last_time / 1e9:.3f}s"
                    else:
                        time_str = "Never"
                    self.debug_table.setItem(row, 1, QTableWidgetItem(time_str))
                    
                    # State
                    state = debug_info.get('last_state', 'UNKNOWN')
                    self.debug_table.setItem(row, 2, QTableWidgetItem(state))
                    
                    # Keys
                    keys = debug_info.get('last_extras_keys', [])
                    self.debug_table.setItem(row, 3, QTableWidgetItem(", ".join(keys[:3])))
                    
                    # Counter
                    counter = debug_info.get('last_counter_value', 0)
                    self.debug_table.setItem(row, 4, QTableWidgetItem(str(counter)))
                    
                    # Busy status
                    busy = "Yes" if station.get('busy', False) else "No"
                    self.debug_table.setItem(row, 5, QTableWidgetItem(busy))
                
                # Update status labels
                total_events = sum(len(station.get('cycle_times', [])) for station in station_states.values())
                self.event_status.setText(f"Events: {total_events}")
                
                if kpi_snapshot.total_completed > 0:
                    self.kpi_status.setText(f"KPI: {kpi_snapshot.throughput_ph:.1f} pph")
                else:
                    self.kpi_status.setText("KPI: No completions")
...
class KpiDashboard(QMainWindow):
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Production Line KPI Dashboard v1.4")
    ...
    def init_ui(self):
        ...
        # Debug toggle button
        debug_button = QPushButton("🐛 Debug Panel")
        debug_button.setObjectName("debugButton")
        debug_button.clicked.connect(self.toggle_debug_panel)
        nav_layout.addWidget(debug_button)
        
        # Version label
        version_label = QLabel(f"v1.4 - {CHART_BACKEND}")
        ...
        self.debug_panel = DebugPanel(self.kpi_calculator)
    ...
    def create_header(self):
        ...
        # Title
        title_label = QLabel("Production Line KPI Dashboard v1.4 - FIXED KPI Math")
    ...
    def create_all_view(self):
        ...
        self.kpi_cards = {
            ...
            'utilization': KpiCard("Utilization", "0%", "", "📈", "#9C27B0")
        }
        ...
        # WIP over time chart (NEW)
        self.wip_chart = create_chart_widget()
        charts_grid.addWidget(self.wip_chart, 0, 1)
        
        # Utilization chart
        self.utilization_chart = create_chart_widget()
        charts_grid.addWidget(self.utilization_chart, 1, 0)
        
        # Completed vs Rejects chart (NEW)
        self.completed_rejects_chart = create_chart_widget()
        charts_grid.addWidget(self.completed_rejects_chart, 1, 1)
    ...
    def create_station_view(self, station_id: str):
        ...
        # ===== STATION-SPECIFIC CHARTS ROW =====
        charts_layout = QHBoxLayout()
        charts_layout.setSpacing(15)
        
        # Chart 1: Station-specific chart
        chart1 = create_chart_widget()
        chart1.setMinimumHeight(400)
        charts_layout.addWidget(chart1, 1)
        
        # Chart 2: Station-specific chart
        chart2 = create_chart_widget()
        chart2.setMinimumHeight(400)
    ...
    def on_kpis_updated(self, snapshot: KpiSnapshot):
        ...
        for station_id in STATIONS.keys():
            station_history = {
                ...
                'utilization': snapshot.station_utilization.get(station_id, 0.0),
                'extra_fields': snapshot.station_extra_fields.get(station_id, {}),
                'metrics': snapshot.station_metrics.get(station_id, {})
            }
            self.station_history[station_id].append(station_history)
        ...
        if self.debug_panel and self.debug_panel.isVisible():
            self.debug_panel.update_debug_info(snapshot)
    ...
    def update_all_view(self, snapshot: KpiSnapshot):
        ...
        self.kpi_cards['utilization'].set_value(f"{snapshot.utilization:.1f}%")
    ...
    def update_all_charts(self):
        """Update charts in ALL view with NEW charts"""
        ...
        try:
            ...
            wip_data = [s.wip for s in self.kpi_history]
            ...
            # Completed vs Rejects over time
            completed_data = [s.total_completed for s in self.kpi_history]
            rejects_data = [s.total_rejected for s in self.kpi_history]
            
            # Generate chart data based on backend
            if CHART_BACKEND == "plotly_webengine":
                # Throughput chart
                throughput_trace, throughput_layout = self.chart_manager.create_throughput_chart_data(times, throughputs)
                self.throughput_chart.update_chart(throughput_trace, throughput_layout)
                
                # WIP chart (NEW)
                wip_trace, wip_layout = self.chart_manager.create_wip_chart_data(times, wip_data)
                self.wip_chart.update_chart(wip_trace, wip_layout)
                
                # Utilization chart
                util_trace, util_layout = self.chart_manager.create_utilization_chart_data(station_names, utilizations)
                self.utilization_chart.update_chart(util_trace, util_layout)
                
                # Completed vs Rejects chart (NEW)
                comp_rej_traces, comp_rej_layout = self.chart_manager.create_completed_rejects_chart_data(
                    times, completed_data, rejects_data
                )
                self.completed_rejects_chart.update_chart(comp_rej_traces, comp_rej_layout)
            else:
                # Throughput chart
                ...
                # WIP chart (NEW)
                wip_data_chart = self.chart_manager.create_wip_chart_data(times, wip_data)
                self.wip_chart.update_chart(wip_data_chart)
                ...
                # Completed vs Rejects chart (NEW)
                comp_rej_data = self.chart_manager.create_completed_rejects_chart_data(
                    times, completed_data, rejects_data
                )
                self.completed_rejects_chart.update_chart(comp_rej_data)
            
        except Exception as e:
            print(f"Error updating ALL view charts: {e}")
            import traceback
            traceback.print_exc()
    
    def update_station_charts(self, station_id: str):
        """Update station-specific charts"""
        if len(self.kpi_history) < 2:
            return
        ...
        try:
            # Get station-specific chart data
            station_charts = self.chart_manager.get_station_charts(station_id, self.kpi_history)
            
            # Update chart 1 with first available chart
            if station_charts and len(station_charts) > 0:
                chart_data = station_charts[0]
                if CHART_BACKEND == "plotly_webengine" and chart_data.get('type') == 'plotly':
                    ui["chart1"].update_chart(chart_data['data'], chart_data['layout'])
                else:
                    ui["chart1"].update_chart(chart_data)
            else:
                # Fallback: cycle time trend
                times = [s.timestamp_s for s in self.kpi_history]
                cycle_times = [s.avg_cycle_times.get(station_id, 0.0) for s in self.kpi_history]
                ...
                if CHART_BACKEND == "plotly_webengine":
                    trace = {
                        ...
                        'mode': 'lines',
                        'name': 'Cycle Time',
                        'line': {'color': STATIONS[station_id]['color'], 'width': 2}
                    }
                    
                    layout = {
                        'title': f'{station_id} Cycle Time Trend',
                        'xaxis': {'title': 'Time'},
                        'yaxis': {'title': 'Cycle Time (ms)'},
                        'showlegend': True,
                        'height': 400,
                    ...
                    ui["chart1"].update_chart([trace], layout)
                else:
                    ui["chart1"].update_chart({
                        'type': 'line',
                        'x': times,
                        'y': cycle_times,
                        'title': f'{station_id} Cycle Time Trend',
                        'xlabel': 'Time',
                        'ylabel': 'Cycle Time (ms)',
                        'color': STATIONS[station_id]['color']
                    })
            
            # Update chart 2 with second available chart
            if station_charts and len(station_charts) > 1:
                chart_data = station_charts[1]
                if CHART_BACKEND == "plotly_webengine" and chart_data.get('type') == 'plotly':
                    ui["chart2"].update_chart(chart_data['data'], chart_data['layout'])
                else:
                    ui["chart2"].update_chart(chart_data)
            else:
                # Fallback: utilization trend
                times = [s.timestamp_s for s in self.kpi_history]
                utilizations = [s.station_utilization.get(station_id, 0.0) for s in self.kpi_history]
                ...
                if CHART_BACKEND == "plotly_webengine":
                    ...
                    layout = {
                        'title': f'{station_id} Utilization Trend',
                        'xaxis': {'title': 'Time'},
                        'yaxis': {'title': 'Utilization (%)', 'range': [0, 100]},
                        'showlegend': True,
                        'height': 400,
                    ...
                    ui["chart2"].update_chart([trace], layout)
                else:
                    ui["chart2"].update_chart({
                        'type': 'line',
                        'x': times,
                        'y': utilizations,
                        'title': f'{station_id} Utilization Trend',
